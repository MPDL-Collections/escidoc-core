...
public class WebServiceLocator extends Service {
     ...
     * @param serviceUrl       The URL to the service as String
     * @param config           The wss4j security configuration
     */
    public WebServiceLocator(final Class serviceInterface, final String serviceName, final String serviceUrl,
     ...
     */
    public WebServiceLocator(final Class serviceInterface, final String serviceName, final EngineConfiguration config)
        ...
        this.serviceInterface = serviceInterface;
        this.serviceName = serviceName;
        this.serviceUrl = (String) config.getService(new QName(serviceName)).getOption("serviceUrl");
     ...
     * @throws ServiceException Thrown if service is not available
     */
    public Remote getService() throws ServiceException {

        final Call call = (Call) this.createCall();
        final Remote stub = call.getService().getPort(this.serviceAddress, this.serviceInterface);
        ...
        try {
            ...
            if (LOGGER.isWarnEnabled()) {
                LOGGER.warn("Error on getting service.");
            }
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug("Error on getting service.", e);
            }
        }
        return stub;
    }
     ...
     * @throws ServiceException Thrown if service is not available
     */
    public Remote getService(final String username, final String password) throws ServiceException {
     ...
     * Get the Service with one type mapping.
     *
     * @param mappings Hashtable of the type mappings as namespace/class pair
     * @return Remote The webservice Remote client
     * @throws ServiceException Thrown if service is not available
     */
    ...
    public Remote getService(final Iterable<BeanMapping> mappings) throws ServiceException {
        ...
        for (final BeanMapping mapping : mappings) {
            ...
            final QName poqn = new QName(mapping.getNamespaceUri(), mapping.getNamespace());
            call.registerTypeMapping(mapping.getBean(), poqn, new BeanSerializerFactory(mapping.getBean(), poqn),
     ...
     *
     * @param mappings Vector of the type mappings
     * @param handle   The security handle to use (will be set as password, username set to "ShibbolethUser")
     * @return Remote The webservice Remote client
     * @throws ServiceException Thrown if service is not available
     */
    public Remote getService(final List<BeanMapping> mappings, final String handle) throws ServiceException {
     ...
     *
     * @param address String for the endpoint address
     */
